...
export default function OurNumbersFigure() {
  ...
  return (
    <div className='container mx-auto max-w-6xl px-4 md:px-6 py-16 font-primary'>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-32 items-center justify-between'>
        {statsData.map((stat, index) => (
          <div key={index} className={`p-1 ${index < statsData.length - 1 ? 'relative' : ''}`}>
            <h3 className='text-5xl mb-2 font-bold text-primary-black'>
              <CountUp
                end={stat.value}
                duration={2.5}
                enableScrollSpy
            ...
            <p className='text-xl min-w-[215px] text-[#475569] font-normal leading-tight'>
              {stat.description}
            </p>
