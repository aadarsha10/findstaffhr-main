...
import { 
  generateIndustryJobApplicationNotificationHtml, 
  generateIndustryJobApplicationNotificationText, 
  generateIndustryJobApplicationConfirmationHtml, 
  generateIndustryJobApplicationConfirmationText 
} from '../../../components/emails/IndustryJobApplicationTemplates';
import { 
  generateMessageId, 
  createEmailHeaders, 
  getLogoUrl 
...
export async function POST(request: Request) {
  try {
    ...
    const formattedSubject = subject || `Job Application for ${category}`;
    ...
    const notificationSubject = `New Job Application: ${category}`;

    // Send notification email using Resend with industry job template
    const { data, error } = await resend.emails.send({
      ...
      text: generateIndustryJobApplicationNotificationText({
        fullName,
        email,
        phone,
        company,
      ...
      html: generateIndustryJobApplicationNotificationHtml({
        fullName,
        email,
        phone,
        company,
    ...
    const confirmationSubject = `Thank you for applying to ${category} jobs - FindStaffHR`;

    // Send confirmation email to the user with industry job template
    const { error: confirmationError } = await resend.emails.send({
      from: fromEmail,
      to: email,
      subject: confirmationSubject,
      headers: confirmationHeaders,
      text: generateIndustryJobApplicationConfirmationText({
        fullName,
        subject: formattedSubject,
        message,
        logoUrl,
      ...
      html: generateIndustryJobApplicationConfirmationHtml({
        fullName,
        subject: formattedSubject,
        message,
        logoUrl,
